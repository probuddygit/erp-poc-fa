  ...
  const items = [
    {
      ...
      warn: health.score < 50,
    },
    {
      label: "SPI",
      value: evm.spi.toFixed(2),
      sub: `${evm.actualPct}% vs ${evm.plannedPct}% planned`,
      good: evm.spi >= 1,
      warn: evm.spi < 0.9,
    },
    {
      ...
      sub: `AC ${fmtCompact(evm.ac)} of ${fmtCompact(evm.bac)}`,
      good: evm.cpi >= 1,
      warn: evm.cpi < 0.9,
    },
    {
      label: "EAC",
      value: fmtCompact(evm.eac),
      sub: `${evm.vac >= 0 ? "Under" : "Over"} by ${fmtCompact(Math.abs(evm.vac))}`,
      good: evm.vac >= 0,
      warn: evm.vac < 0,
    },
    {
      label: "Forecast Finish",
      value: new Date(evm.forecastFinish).toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "short",
      }),
      sub: evm.scheduleSlipDays > 0 ? `+${evm.scheduleSlipDays}d slip` : "On baseline",
      good: evm.scheduleSlipDays <= 0,
      warn: evm.scheduleSlipDays > 14,
    },
    {
      label: "Forecast Margin",
      value: `${evm.profitability}%`,
      sub: "on contract value",
      good: evm.profitability >= 12,
      warn: evm.profitability < 5,
    },
  ];
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
      {items.map((it) => (
        <div key={it.label} className="rounded-xl border bg-card p-3">
          ...
          <div
            className={cn(
              "mt-1 font-display text-xl font-semibold capitalize",
              it.warn ? "text-rose-600" : it.good ? "text-emerald-600" : "text-amber-600",
            )}
          >
            {it.value}
          </div>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            {health.drivers.length === 0 && (
              <span className="text-xs text-muted-foreground">
                No detractors — all indicators within tolerance.
              </span>
            )}
          </div>
        </div>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Button
            ...
            onClick={() => {
              ...
              toast.success(`Rescheduled ${moves.length} item(s)`, {
                description: moves.slice(0, 3).join(" · "),
              });
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              ...
              toast[n ? "success" : "info"](
                n ? `Triaged and assigned ${n} issue(s)` : "All issues already triaged",
              );
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              emerging.forEach((r) =>
                upsertProjectRecord("risks", { ...r, source: undefined }, i.project.id),
              );
            ...
            onClick={() => {
              ...
              overdue.forEach((a) => {
                if (a.milestone.status !== "at-risk")
                  upsertProjectRecord(
                    "milestones",
                    { ...a.milestone, status: a.dueInDays < 0 ? "missed" : "at-risk" },
                    i.project.id,
                  );
              });
          ...
          >
            <CalendarClock className="h-4 w-4 text-primary" /> Escalate milestone alerts (
            {alerts.length})
          </Button>
        </div>
        ...
        <div className="grid gap-3 border-t pt-3 text-xs sm:grid-cols-3">
          ...
          <div>
            ...
            <div className="truncate text-muted-foreground">
              {conflicts[0] ? `${conflicts[0].name} @ ${conflicts[0].totalAllocation}%` : "None"}
            </div>
          </div>
          <div>
...
export function AiPlanDialog({
  ...
  const template: ProjectTemplate =
    PROJECT_TEMPLATES.find((t) => t.id === templateId) ?? rec.template;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Recommended methodology for {project.code} — review, switch template or apply to
        ...
        <div className="max-h-[60vh] space-y-4 overflow-y-auto pr-1">
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            {[
              [
                "Phases / tasks",
                `${plan.wbs.length} / ${plan.wbs.reduce((s, p) => s + p.children.length, 0)}`,
              ],
          ...
          <div className="rounded-lg border">
            <div className="border-b bg-muted/40 px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Generated schedule · estimated finish{" "}
              {new Date(plan.estimatedFinish).toLocaleDateString("en-IN", {
                day: "2-digit",
                month: "short",
                year: "numeric",
              })}
          ...
          <Button
            ...
            onClick={() => {
              ...
              toast.success("Project plan generated", {
                description: Object.entries(created)
                  .map(([k, v]) => `${v} ${k}`)
                  .join(" · "),
              });
            }}
