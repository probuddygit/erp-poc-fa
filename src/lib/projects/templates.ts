...
const BIW: ProjectTemplate = {
  ...
  description:
    "Full body-in-white line: concept, simulation, fixture build, robot integration, tryout and buy-off.",
  keywords: ["biw", "body", "weld", "line", "framing", "underbody"],
  minValue: 30_000_000,
  phases: [
    {
      name: "Concept & Contract Freeze",
      duration: 0.1,
      weight: 8,
  ...
  risks: [
    {
      title: "Customer input data changes after design freeze",
      category: "Schedule",
      probability: 4,
      impact: 4,
      mitigation:
        "Lock the input register at kick-off; route all later changes through the change control board.",
    },
    {
      title: "Robot / long-lead equipment delivery slippage",
      category: "Supplier",
      probability: 3,
      impact: 5,
      mitigation: "Place long-lead POs in week 1 and track weekly with Procurement.",
    },
    {
      title: "Dimensional non-conformance at tryout",
      category: "Quality",
      probability: 3,
      impact: 4,
      mitigation: "Stage-wise CMM checks at fixture and assembly stages before tryout.",
    },
    {
      title: "Site readiness delay at customer plant",
      category: "Schedule",
      probability: 3,
      impact: 3,
      mitigation: "Joint site-readiness checklist signed 4 weeks before dispatch.",
    },
    {
      title: "Safety incident during installation",
      category: "Safety",
      probability: 2,
      impact: 5,
      mitigation: "Site-specific HIRA, permit-to-work and daily toolbox talks.",
    },
  ],
  docFolders: [
    "01 Contract & Charter",
  ...
  qualityChecklist: [
    "Input data register signed",
    "Design review closure",
    "Incoming material inspection",
    "Fixture CMM report",
    "Electrical safety test",
    "Cycle time validation",
    "Customer buy-off checklist",
  ],
};
...
const ROBOT_CELL: ProjectTemplate = {
  ...
  risks: [
    {
      title: "Scope creep on cell requirements",
      category: "Cost",
      probability: 3,
      impact: 4,
      mitigation: "URS baseline with signed change control.",
    },
    {
      title: "Controls hardware lead time",
      category: "Supplier",
      probability: 3,
      impact: 4,
      mitigation: "Order controls hardware at design release.",
    },
    {
      title: "Cycle time not met at FAT",
      category: "Technical",
      probability: 2,
      impact: 5,
      mitigation: "Simulate cycle time at design stage and validate at trial run.",
    },
  ],
  docFolders: [
    "01 Contract",
...
const RETROFIT: ProjectTemplate = {
  ...
  description:
    "Brownfield upgrade, line rebalancing or controls migration executed inside a customer shutdown window.",
  ...
  risks: [
    {
      title: "Shutdown window shorter than planned",
      category: "Schedule",
      probability: 4,
      impact: 5,
      mitigation: "Pre-fabricate off-site and stage a rehearsal before the window.",
    },
    {
      title: "As-built differs from documentation",
      category: "Technical",
      probability: 4,
      impact: 3,
      mitigation: "Physical survey with photographic record before design.",
    },
    {
      title: "Working at height / hot work incident",
      category: "Safety",
      probability: 2,
      impact: 5,
      mitigation: "Permit-to-work and dedicated safety steward on shift.",
    },
  ],
  docFolders: [
    "01 Contract",
...
export function recommendTemplate(
  ...
  const scored = PROJECT_TEMPLATES.map((t) => {
    ...
    if (input.value >= t.minValue && t.minValue > 0) {
      score += 20;
      reasons.push(
        `Contract value ₹${Math.round(input.value).toLocaleString("en-IN")} fits ${t.name} band`,
      );
    }
...
export function generatePlan(project: Project, template: ProjectTemplate): GeneratedPlan {
  ...
  const totalDays = Math.max(
    Math.round(
      (new Date(project.endDate).getTime() - new Date(project.startDate).getTime()) / 86_400_000,
    ),
    30,
  );
  ...
  const budget = (Object.entries(template.budgetSplit) as Array<[string, number]>).map(
    ([category, pct]) => ({
      category,
...
export function applyPlan(
  ...
  if (o.wbs) {
    ...
    for (const phase of plan.wbs) {
      const parentId = upsertProjectRecord(
        "wbs",
        {
          code: phase.code,
          name: phase.name,
    ...
    plan.docFolders.forEach((f) =>
      upsertProjectRecord(
        "docs",
        {
          ...
          notes: "Auto-created project document folder",
        },
        projectId,
      ),
    );
...
export function autoPlanProject(projectId: string, templateId?: string) {
  ...
  const rec = templateId
    ? {
        template: PROJECT_TEMPLATES.find((t) => t.id === templateId) ?? PROJECT_TEMPLATES[0],
        confidence: 100,
        reasons: ["Template selected by the project manager"],
        alternatives: [] as ProjectTemplate[],
      }
